...
\label{alg:alphaflow}
\begin{algorithmic}[1]
\Require $x \sim p_{\text{data}}$, $\epsilon \sim \mathcal{N}(0,I)$, average-velocity network $u_\theta$, iteration $k$, schedule $\alpha(k)$, optimizer
\State Sample $r, t \sim \mathcal{U}[0,1]$ with $r < t$
