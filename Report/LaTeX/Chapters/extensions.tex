...
Meta Flow Maps (MFMs) \citep{potaptchik2025meta} address this by extending the deterministic flow map formalism into the \emph{stochastic regime}. A stochastic flow map is defined as a transformation that maps an exogenous noise source $\epsilon$ and an intermediate state $(t, x)$ directly to a sample $x_1 \sim p_{1|t}(\cdot | x)$. By varying the noise input, the map can generate arbitrarily many i.i.d. draws from the posterior in a single step.
...
Empirically, MFMs achieve competitive sample quality while markedly reducing computation for both steering and fine-tuning, outperforming Best-of-1000 baselines on ImageNet across multiple reward functions at a fraction of the compute.
