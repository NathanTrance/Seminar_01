...
\begin{itemize}
    ...
    \item We analyze four key methodological advances---consistency flow matching, mean flows, terminal velocity matching, and flow-map-based methods---comparing their training requirements, theoretical guarantees, and empirical performance.
...
The remainder of this survey is organized as follows. \Cref{sec:related} reviews related work, including the foundations of flow matching and conditional flow matching (\cref{subsec:fm-cfm}), traditional acceleration strategies such as caching and step distillation (\cref{subsec:traditional}), and related paradigms including consistency models and flow-map distillation (\cref{subsec:related-methods}). \Cref{sec:onestep} presents the four core families of one-step methods: consistency flow matching, mean flows, terminal velocity matching, and flow-map-based approaches. \Cref{sec:experiments} describes experimental setups and baselines, while \Cref{sec:results} synthesizes the empirical results. \Cref{sec:conclusion} concludes with future directions.
