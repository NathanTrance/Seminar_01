...
\paragraph{Velocity consistency.} Given a flow trajectory $\gamma_x(t)$ with learned velocity $v_\theta(t, \gamma_x(t))$, the core idea is to require the velocity to be constant along each trajectory:
\begin{equation}
    v(t, \gamma_x(t)) = v(r, \gamma_x(r)) \quad \forall t, r \in [0,1].
    \label{eq:cfm-cond1}
\end{equation}
...
\begin{equation}
    \gamma_x(t) + (1-t) \, v(t, \gamma_x(t)) = \gamma_x(r) + (1-r) \, v(r, \gamma_x(r)),
    \label{eq:cfm-cond2}
\end{equation}
...
\end{equation}
In practice, the training batch mixes a fraction $k$ of self-consistency samples ($r < t$) with $1 - k$ flow matching samples ($r = t$), yielding a combined objective trained jointly in a single phase.
...
\input{Pseudocode/shortcut_models.tex}

\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{Figures/Shortcut.png}
  \caption{Overview of shortcut model training \citep{frans2024shortcut}. At $d \approx 0$, the shortcut objective is equivalent to the flow matching objective and can be trained by regressing onto empirical $(\epsilon, x)$ pairs. Targets for larger $d$ are constructed by concatenating a sequence of two $d/2$ shortcuts. Both objectives are trained jointly in a single end-to-end phase.}
  \label{fig:shortcut}
\end{figure}
...
\begin{figure}[t]
  ...
  \caption{The field of average velocity $u(z, r, t)$ \citep{geng2025mean}. \textbf{Leftmost:} While the instantaneous velocity $v$ determines the tangent direction of the path, the average velocity $u(z, r, t)$ is generally not aligned with $v$. The average velocity is aligned with the displacement, which is $(t - r) \, u(z, r, t)$. \textbf{Right three subplots:} The field $u(z, r, t)$ is conditioned on both $r$ and $t$, shown here for $t = 0.5$, $0.7$, and $1.0$.}
